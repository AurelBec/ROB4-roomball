#ifndef TEXTMENUOPTION_HPP
#define TEXTMENUOPTION_HPP

#include "../Texture.hpp"
#include "../TextureText.hpp"


class TextureMenuOption : public Texture
{
public:
	TextureMenuOption()
	: m_thickness(H_BORD)
	...
	, m_dim(W_WINDOW - 4 * (FONT_SIZE + 5) + 2*H_BORD, H_WINDOW - 4 * (FONT_SIZE + 5) + 2*H_BORD)
	{
		...
		m_option[0] = TextureText(sf::Color::White, "OPTIONS", 3*FONT_SIZE, W_WINDOW/2, H_WINDOW / 4);
		m_option[1] = TextureText(sf::Color::White, "Retour",  2*FONT_SIZE, W_WINDOW/2, H_WINDOW/4 + 35*FONT_SIZE);
	}
